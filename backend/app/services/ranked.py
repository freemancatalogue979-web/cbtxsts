...
def _launch_match(db: Session, course_id: int | None, waiting: list[RankedQueue]) -> list[Event]:
    ...
    if len(player_ids) < MIN_PLAYERS:
        return []
    # Multiplayer rule: every player gets their own unique set. If the bank
    # cannot cover the full QUESTION_COUNT for everyone without reuse, shrink
    # the per-player count (never share, never silently repeat); if even one
    # unique question each is impossible, the queue simply keeps waiting.
